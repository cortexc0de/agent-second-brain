...
async def cmd_start(message: Message) -> None:
    ...
    await message.answer(
        ...
        "/decisions [limit] - обзор решений по attention и review status\n"
        "/decide_trace - посмотреть trace принятого решения\n"
        "/review [limit] - очередь due reviews и их review status\n"
        "/review_trace [limit] - trace доставки review для длинных очередей\n"
...
async def cmd_help(message: Message) -> None:
    ...
    await message.answer(
        ...
        "/decisions [limit] - обзор решений по приоритету и review status\n"
        "/decide_trace - посмотреть reasoning trace решения\n"
        "/review [limit] - посмотреть очередь due reviews\n"
        "/review_trace [limit] - посмотреть delivery trace review\n"
        "/do - выполнить произвольный запрос\n"
        "/weekly - недельный дайджест\n\n"
        "<i>Показывает секции: Needs Attention, Due Soon, Stable</i>\n"
        "<i>Для /review можно задать limit: /review 10</i>\n"
        "<i>Для /review_trace можно задать limit: /review_trace 10</i>\n"
